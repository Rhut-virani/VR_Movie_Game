...
export default class Clues extends React.Component {
	...
	animateImage = () => {
		...
		Animated.spring(
			...
			{
				toValue: 0.92, // Animate to different size
	...
	render() {
		...
		const director =
			moviestate.credits.crew[0].job + " : " + moviestate.credits.crew[0].name;
		...
		const plotArray = plotSplit.filter((element, index) => {
			return index < 15;
		});
		plotArray.push(".....");
				...
				<View style={styles.clueContainer}>
					<View style={styles.queContainer}>
						<View style={styles.que1}>
							<Text style={styles.quetext}>Genre : {genre}</Text>
						</View>
						<View style={styles.que2}>
							<Text style={styles.quetext}>Release Date : {releaseDate}</Text>
						</View>
						<View style={styles.que1}>
							<Text style={styles.quetext}>{director}</Text>
						</View>
						<View style={styles.que2}>
							<Text style={styles.quetext}>
								{tagline ? "TagLine : " + tagline : "No Tagline found"}
							</Text>
						</View>
						<View style={styles.que1}>
							{plotValue ? (
								...
								<GazeButton
									...
									render={(remainingTime, isGazed) => (
										<Text style={styles.quetext}>
											Click for Movie Plot (10 Points)
										</Text>
									)}
								/>
							)}
					...
					<View style={styles.ansContainer}>{ansJSX}</View>
				</View>
			);
		}
	}
